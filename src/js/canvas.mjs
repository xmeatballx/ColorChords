...
export const drawCanvas = () => {
    ...
    state.notes.forEach((note, i) => {
        const x = canvas.width / 2 + Math.sin(Math.PI * 2 * note.interval + (state.transpose / 180 * Math.PI)) * 204 * note.velocity
        const y = canvas.height/2 + Math.cos(Math.PI * 2 *note.interval+(state.transpose/180*Math.PI)) * 204 * note.velocity
        ctx.beginPath();
