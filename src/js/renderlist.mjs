...
import {attachListeners} from './eventhandler.mjs'
...
let styles = {
  ...
  rotationValue: "rotation_value text-sm text-mid 2xl:text-2xl",
  rotation: "slider light w-full mt-3",
  noteList: "notes md:overflow-y-scroll md:scrollbar md:scrollbar-thin md:h-full w-full mb-3",
...
function renderListTemplate(state) {
    ...
    <li class="${styles.note}" data-index=${index}>
			<div class=${styles.controls}>
				<div class="interval-wrapper">
					<select name="interval" class="${styles.interval}" data-index=${index} data-parameter="interval">
						<option value="1" ${note.interval == 1 ? 'selected' : ''} class="text-sm">Fundamental</option>
						<option value="1.067" ${note.interval == 1.067 ? 'selected' : ''} class="text-sm">Minor Second</option>
						<option value="1.125" ${note.interval == 1.125 ? 'selected' : ''} class="text-sm">Major Second</option>
						<option value="1.200" ${note.interval == 1.200 ? 'selected' : ''} class="text-sm">Minor Third</option>
						<option value="1.250" ${note.interval == 1.250 ? 'selected' : ''} class="text-sm">Major Third</option>
						<option value="1.333" ${note.interval == 1.333 ? 'selected' : ''} class="text-sm">Perfect Fourth</option>
						<option value="1.414" ${note.interval == 1.414 ? 'selected' : ''} class="text-sm">Tritone</option>
						<option value="1.500" ${note.interval == 1.500 ? 'selected' : ''} class="text-sm">Perfect Fifth</option>
						<option value="1.600" ${note.interval == 1.600 ? 'selected' : ''} class="text-sm">Minor Sixth</option>
						<option value="1.666" ${note.interval == 1.666 ? 'selected' : ''} class="text-sm">Major Sixth</option>
						<option value="1.777" ${note.interval == 1.777 ? 'selected' : ''} class="text-sm">Minor Seventh</option>
						<option value="1.875" ${note.interval == 1.875 ? 'selected' : ''}class="text-sm">Major Seventh</option>
					</select>
				</div>
				<div class="${styles.colorPreview}" style="background: rgb(${note.color[0]} ${note.color[1]} ${note.color[2]})"></div>
				...
				<div class="${styles.velocityContainer}">
					...
					<input type="range" name="velocity" class="${styles.velocity}" min=0.0 max=1.0 value=${note.velocity} step="0.1" data-index=${index} data-parameter="velocity"/>
				</div>
			</div>
  ...
  container.innerHTML = transposeControl + listItems.join('') + '</ul>' + addNoteButton;
